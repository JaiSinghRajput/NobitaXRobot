...
from nobita.core import pyro_cooldown
from nobita.helper import check_time_gap, fetch, post_to_telegraph, use_chat_lang
from nobita.vars import BARD_API, COMMAND_HANDLER, OPENAI_API, SUDO
...
async def bard_chatbot(_, ctx: Message, strings):
    ...
    try:
        req = await fetch.get(
            f"https://nobiapi.eu.org/bard?input={ctx.text.split(maxsplit=1)[1]}&key={BARD_API}"
        )
...
async def openai_chatbot(_, ctx: Message, strings):
    ...
    try:
        ...
        answerlink = await post_to_telegraph(
            False, "nobita ChatBot ", html.escape(f"<code>{answer}</code>")
        )
        await msg.edit_msg(
