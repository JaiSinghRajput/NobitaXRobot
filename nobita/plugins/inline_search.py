...
from nobita import BOT_USERNAME, app, user
from nobita.helper import GENRES_EMOJI, fetch, post_to_telegraph, search_jw
from nobita.plugins.dev import shell_exec
from nobita.plugins.misc_tools import calc_btn
from nobita.vars import USER_SESSION
...
async def inline_menu(self, inline_query: InlineQuery):
    if inline_query.query.strip().lower().strip() == "":
        ...
        msg = f"""
**[nobita✨](https://github.com/jaisingh007):**
...
"""
        ...
        answerss = [
            ...
            InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    f"<b>Github Repo @{BOT_USERNAME}</b>\n\nhttps://github.com/jaisingh007/NobitaXRobot"
                ),
        ...
        for method in parsemethod:
            if kueri.lower() in method.lower():
                ...
                buttons.row(
                    InlineButton("Give Coffee", url="https://Nobi-Pro.eu.org"),
                )
        ...
        query = inline_query.query.split(None, 1)[1].strip()
        search_results = await fetch.get(f"https://nobiapi.eu.org/pypi?q={query}")
        ...
        search_results = await fetch.get(
            f"https://nobiapi.eu.org/imdb-search?q={movie_name}"
        )
...
async def imdb_inl(_, query):
    ...
    if cbuser == f"{query.from_user.id}":
        try:
            ...
            res_str += "<b>©️ IMDb by</b> @nobitaRoBot"
