...
from nobita.helper import post_to_telegraph, progress_for_pyrogram, runcmd
from nobita.helper.localization import use_chat_lang
from nobita.helper.mediainfo_paste import mediainfo_paste
...
{out or 'Not Supported'}
    """
        try:
            link = await mediainfo_paste(out, "nobita Mediainfo")
            markup = InlineKeyboardMarkup(
                [[InlineKeyboardButton(text=strings("viweb"), url=link)]]
            )
        except:
            try:
                link = await post_to_telegraph(False, "nobita MediaInfo", f"<code>{body_text}</code>")
                markup = InlineKeyboardMarkup(
                    [[InlineKeyboardButton(text=strings("viweb"), url=link)]]
                )
            except:
        ...
        with io.BytesIO(str.encode(body_text)) as out_file:
            out_file.name = "nobita_Mediainfo.txt"
            ...
            try:
                ...
                try:
                    link = await post_to_telegraph(
                        False, "nobita MediaInfo", body_text
                    )
                    markup = InlineKeyboardMarkup(
                        [[InlineKeyboardButton(text=strings("viweb"), url=link)]]
                    )
