...
from nobita.helper import Cache, fetch
from nobita.plugins.web_scraper import split_arr
...
async def getDataPypi(msg, kueri, CurrentPage, user):
    if not PYPI_DICT.get(msg.id):
        pypijson = (await fetch.get(f"https://nobiapi.eu.org/pypi?q={kueri}")).json()
