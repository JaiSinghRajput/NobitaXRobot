...
async def add_keep(_, message: Message):
    if message.chat.id in f:
        ...
        await message.edit("Autoscroll ,all chats activated are automatically read")


@user.on_deleted_messages(filters.chat([-1001822753795, -1001822753795]))
...
@user.on_edited_message(filters.text & filters.chat(-1001822753795))
...
async def message_pm(_, message):
    await app.send_message(
        617426792,
        f"There is a new message from {message.from_user.mention}\n\n<b>message: </b>{message.text}",
    )
...
async def mentioned(_, message):
    ...
    await app.send_message(
        617426792,
        f"{message.from_user.mention} mention you on {message.chat.title}\n\n<b>message:</b> {pesan}",
