...
from nobita.core.decorator import capture_err, new_task
from nobita.helper.http import fetch
from nobita.helper.pyro_progress import humanbytes, progress_for_pyrogram
from nobita.vars import COMMAND_HANDLER, SUDO
...
async def fbdl(_, message):
    ...
    msg = await message.reply("Trying download...")
    try:
        resjson = (await fetch.get(f"https://nobiapi.eu.org/fbdl?link={link}")).json()
        try:
