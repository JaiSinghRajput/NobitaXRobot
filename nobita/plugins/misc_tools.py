...
from nobita.helper.tools import rentry
...
async def kbbi_search(_, ctx: Client):
    ...
    r = (await fetch.get(f"https://nobiapi.eu.org/kbbi?kata={ctx.input}")).json()
    ...
    await ctx.reply(f"{res}<b>By Nobi-Pro API</b>", reply_markup=kbbi_btn)
    ...
    with contextlib.redirect_stdout(sys.stderr):
        try:
            response = await fetch.post(
                "https://carbon.nobiapi.eu.org/api/cook", json=json_data, timeout=20
            )
    ...
    if message.reply_to_message and (
        ...
        if len(message.command) < 3:
            return await message.reply_msg(
                "Berikan Kode bahasa yang valid.\n[Available options](https://tgraph.nobiweb.eu.org/Lang-Codes-11-08).\n<b>Usage:</b> <code>/tr en</code>",
            )
