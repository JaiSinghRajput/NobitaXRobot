...
from nobita.helper import Cache, Kusonime, fetch, use_chat_lang, post_to_telegraph
...
async def getDataTerbit21(msg, kueri, CurrentPage, strings):
    if not SCRAP_DICT.get(msg.id):
        with contextlib.redirect_stdout(sys.stderr):
            try:
                if kueri:
                    terbitjson = await fetch.get(f"{web['nobiapi']}/terbit21?q={kueri}")
                else:
                    terbitjson = await fetch.get(f"{web['nobiapi']}/terbit21")
...
async def getDatalk21(msg, kueri, CurrentPage, strings):
    if not SCRAP_DICT.get(msg.id):
        with contextlib.redirect_stdout(sys.stderr):
            try:
                if kueri:
                    lk21json = await fetch.get(f"{web['nobiapi']}/lk21?q={kueri}")
                else:
                    lk21json = await fetch.get(f"{web['nobiapi']}/lk21")
...
async def getDataPahe(msg, kueri, CurrentPage, strings):
    if not SCRAP_DICT.get(msg.id):
        with contextlib.redirect_stdout(sys.stderr):
            try:
                if kueri:
                    pahejson = await fetch.get(f"{web['nobiapi']}/pahe?q={kueri}")
                else:
                    pahejson = await fetch.get(f"{web['nobiapi']}/pahe")
    ...
    with contextlib.redirect_stdout(sys.stderr):
        try:
            ...
            if "/tv/" in link:
                ...
                for i in result:
                    msg += str(f"{i}\n")
                link = await post_to_telegraph(False, "nobita NoDrakor", msg)
            ...
            res = "".join(f"{i.text}\n{i['href']}\n\n" for i in res)
            if len(res) > 3500:
                link = await post_to_telegraph(False, "nobita NoDrakor", res)
                return await callback_query.message.edit_msg(
                    strings("res_scrape").format(link=link, kl=link), reply_markup=keyboard
                )
            await callback_query.message.edit_msg(
