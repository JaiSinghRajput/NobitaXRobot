...
from nobita.helper import fetch, post_to_telegraph, rentry
...
async def wastepaste(_, message):
    ...
    msg = await message.reply_msg("`Pasting to nobiBin...`")
    data = ""
    limit = 1024 * 1024
    if reply and reply.document:
        if reply.document.file_size > limit:
    ...
    try:
        ...
        response = await fetch.post("https://paste.nobi.eu.org/api/new", json=json_data)
        url = f"https://paste.nobi.eu.org/{response.json()['id']}"
    except Exception as e:
        return await msg.edit_msg(f"ERROR: {e}")
    ...
    pasted = f"**Successfully pasted your data to nobiBin<a href='{url}'>.</a>\n\nPaste by {uname}**"
    await msg.edit_msg(pasted, reply_markup=InlineKeyboardMarkup(button))
