...
from nobita import BOT_NAME, app, botStartTime, nobita_version, user
from nobita.core.decorator import new_task
from nobita.helper.eval_helper import format_exception, meval
from nobita.helper.functions import extract_user, extract_user_and_reason
from nobita.helper.http import fetch
from nobita.helper.human_read import get_readable_file_size, get_readable_time
from nobita.helper.localization import use_chat_lang
from nobita.vars import AUTO_RESTART, COMMAND_HANDLER, LOG_CHANNEL, SUDO
...
async def log_file(_, ctx: Message, strings):
    ...
    if len(ctx.command) == 1:
        try:
            with open("nobitaLogs.txt", "r") as file:
                content = file.read()
            data = {
                "value": content,
            }
            pastelog = await fetch.post("https://paste.nobiapi.eu.org/save", data=data, follow_redirects=True)
            await msg.edit_msg(f"<a href='{pastelog.url}'>Here the Logs</a>\nlog size: {get_readable_file_size(os.path.getsize('nobitaLogs.txt'))}")
        except Exception:
            await ctx.reply_document(
                "nobitaLogs.txt",
                caption="Log Bot NobitaXRobot",
...
async def donate(self: Client, ctx: Message):
    try:
        await ctx.reply_photo(
            "https://img.nobiweb.eu.org/file/9427d61d6968b8ee4fb2f.jpg",
            caption=f"Hi {ctx.from_user.mention}, If you find this bot useful, you can make a donation to the account below. Because this bot server uses VPS and is not free. Thank You..\n\n<b>Indonesian Payment:</b>\n<b>QRIS:</b> https://img.nobiweb.eu.org/file/b1c86973ae4e55721983a.jpg (nobi Store)\n<b>Mayar:</b> https://jaisingh007.mayar.link/payme\n<b>Bank Jago:</b> 109641845083 (nobi Aris M)\n\nFor international people can use PayPal to support me or via GitHub Sponsor:\nhttps://paypal.me/jaisingh007\nhttps://github.com/sponsors/jaisingh007\n\n<b>Source:</b> @BeriKopi",
        )
...
async def update_restart(_, ctx: Message, strings):
    ...
    os.execvp(sys.executable, [sys.executable, "-m", "nobita"])
